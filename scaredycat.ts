...
TODO
- Parameterize aggression
- Optimize parameters
- Be more conservative when about to lose
...
class Bot {
	...
	makeMove(gamestate: Gamestate): Moves {
		...
		const thresholds = new RPSDWArray(
			valuesForMe.rock,
			valuesForMe.rock + valuesForMe.paper,
			valuesForMe.rock + valuesForMe.paper + valuesForMe.scissors,
			valuesForMe.rock + valuesForMe.paper + valuesForMe.scissors + valuesForMe.dynamite,
			1
		);
		
		const choice = Math.random();
		let move;
		if (choice < thresholds.rock) {
			move = Moves.Rock
		} else if (choice < thresholds.paper) {
			move = Moves.Paper
		} else if (choice < thresholds.scissors) {
			move = Moves.Scissors
		} else if (choice < thresholds.dynamite) {
			move = Moves.Dynamite
		} else if (choice < thresholds.waterbomb) {
			move = Moves.Waterbomb
		}
		
		this.valueOfLastRound = this.valueOfCurrentRound;
		console.log('Four ' + move + this.me.dynamitesLeft);
		return move
	}
	
	get estimatedRoundsLeft(): number {
		let fractionWon: number;
		const winningPlayer = this.me.points > this.them.points ? 1 : 2;
		const winningScore = this.player(winningPlayer).points;
...
class Player {
	...
	addRound(round: Round): void {
		...
		if (Bot.resultOfRound(round) === this.number) {
			this.points += this.parent.valueOfLastRound;
		}
	}
